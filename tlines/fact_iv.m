function [ f0, f1, f2, f3, f4 ] = fact_iv(tl,iobs)
% [ f0, f1, f2, f3, f4 ] = fact_iv(tl,iobs)
%
...
% allows to find the current given the voltage source.
...
% Common multipliers
m0 = tl.Y0(:,n)/2;
m14 = m0./(1-tl.Gls(:,n).*tl.Ggr(:,n).*tl.t(:,n));

% Factors corresponding to the reflected rays
f1 = -tl.Ggr(:,n).*m14;
f2 = -tl.Gls(:,n).*m14;
f3 = f4 = tl.Gls(:,n).*tl.Ggr(:,n).*m14;
