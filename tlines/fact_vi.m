function [ f0, f1, f2, f3, f4 ] = fact_vi(tl,iobs)
% [ f0, f1, f2, f3, f4 ] = fact_vi(tl,iobs)
%
...
% allows to find the voltage given the shunt current source.
% The function results are five multipliers, using these multipliers
% the voltage can be found according to the formula below.
%   v = f0*exp(-k*abs(z-z')) + f1*exp(-k*(2*z(i+1)-(z+z'))) 
...
% Common multipliers
m04 = tl.Z0(:,n)/2;
m14 = 1./(1-tl.Gls(:,n).*tl.Ggr(:,n).*tl.t(:,n));
